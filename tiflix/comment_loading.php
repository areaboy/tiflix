...
$("#comment_total_"+postid).text(comment);
 ...
 "<img style='border-style: solid; border-width:3px; border-color:#ec5574; width:40px;height:40px; max-width:40px;max-height:40px;border-radius: 50%;' src=" + comment_photo +" /><br>" +
...
<script>
 ...
 $('.loadmore_comment_btn').click(function () {

              

                    var row_limitc = Number($('#row_limitc').val());
                    var total_countc = Number($('#total_countc').val());
		    var querytotalc  = total_countc;
                   var rowpagec = <?php echo $rowpage_video_post_comment; ?>;

  //var rowpagec = 2;
                    row_limitc = row_limitc + rowpagec;
var post_id= this.id;

//alert('row:' +row_limitc);
//alert('query:' +querytotalc);					
					 if(row_limitc >= querytotalc){
               
                   alert('No More Comment to Load for this Video/Posts');
$("#nomore_content_checkc").html("<div style='background:purple;color:white;padding:10px;bottom:0'>No More Content to Load <br> <center><button style='background:#3b5998;border:none;color:white;padding:10px;cursor:pointer' title='Refresh Page' class='reloadData'>Refresh Page</button></center> </div>");   
$('#loader_comments').hide();
                }

                    if(row_limitc <= querytotalc){
                        $('#row_limitc').val(row_limitc);
$("#loader_comments").fadeIn(400).html('<br><div style="color:black;background:white;padding:10px;">Please Wait. Loading Comments <i class="fa fa-spinner fa-spin" style="font-size:20px"></i></div>');

                        $.ajax({
                            url: 'comments_paginate.php',
                            type: 'post',
                            data: {row_limit:row_limitc, post_id:post_id},
                            success: function(response){
                                $(".postx:last").after(response).show().fadeIn("slow");
$('#loader_comments').hide();
                            }
                        });
                    }
...
$res= $db->prepare("SELECT count(*) as totalcount FROM comments WHERE postid=:postid");
$res->execute(array(':postid' =>$postid));
$t_row = $res->fetch();
$totalcount_comment = $t_row['totalcount'];
...
$commentsData->bindValue(':row1', (int) trim($limit), PDO::PARAM_INT);
...
//$count_comment = $commentsData->rowCount();
...
$verified = htmlentities(htmlentities($rowComment['verified'], ENT_QUOTES, "UTF-8"));
...
<br>
<div id="loader_comments" class="loader_comments"></div>
<div id="nomore_content_check_no1"></div>
            <input type="hidden" id="row_limitc" value="0">
            <input type="hidden" id="total_countc" value="<?php echo $totalcount_comment; ?>">
<button class="loadmore_comment_btn loadmore_css" id="<?php echo $postid; ?>">Load More Comments</button>
