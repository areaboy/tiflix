...
// get total count
$pstmt = $db->prepare('SELECT * FROM subscription where userid = :userid');
...
$sql_query .= 'WHERE (userid =:userid) AND  (invoice_id LIKE "%'.$search_value.'%"  OR invoice_amount LIKE "%'.$search_value.'%" OR invoice_url LIKE "%'.$search_value.'%" OR invoice_status LIKE "%'. $search_value.'%")';
